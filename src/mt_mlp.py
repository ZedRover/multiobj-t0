...
def backtest_stocks(
    ...
    with th.no_grad():
        for code in stk_list:
            ...
            y_pred = model(x_test)["ret"].cpu().detach().numpy()
...
class CustomValidationCallback(pl.Callback):
    ...
    def perform_validation(self, model):
        ...
        self.logger.log_metrics(
            {
                "test/pIC_ckpt": result.pIC.mean(),
                "test/bIC_ckpt": result.bIC.mean(),
                "test/q01_ckpt": result.q01.mean(),
                "test/q99_ckpt": result.q99.mean(),
            },
...
if __name__ == "__main__":
    ...
    ## Model
    # tower_sizes = {"ret": [64, 32, 1], "mean": [40, 1], "var": [40, 1], "rv": [40, 1]}
    # tower_sizes = {"ret": [40, 1], "rv": [40, 1]}
    tower_sizes = {"ret": [64, 1], "mean": [32, 1], "rv": [32, 1]}
    ...
    experiment_name = (
        f"f{args.fold}_n{len(stk_list)}_b{args.batch_size}_{args.cur}-{args.fut}"
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="model_{valid/ic-ret:.3f}",
        save_top_k=1,
    )
