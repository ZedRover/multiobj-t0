...
args.add_argument("--period", "-p", type=int, default=0)
args.add_argument("--horizon", "-ho", type=int, nargs="+", default=[0, 1, 2])
args.add_argument("--labels", type=int, nargs="+", default=[0, 2])
args = args.parse_args()


task_period = [(0, 60), (60, 120), (60, 300), (60, 600), (60, 6000), (120, 180)]
task_horizon = [120, 180, 300, 600, 6000]
task_label = ["mean", "var", "vol", "min", "max", "gap"]

task_period = task_period[args.period]
task_horizon = [task_horizon[i] for i in args.horizon]
task_label = [task_label[i] for i in args.labels]

label_names = ["ret"] + [f"ret_{i}" for i in task_horizon]
for i in range(len(task_label)):
    label_names.append(f"{task_label[i]}_{task_period[0]}-{task_period[1]}")


def get_label(code, label_names):
    ...
    res = np.concatenate(labels, axis=1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for code in tqdm(codes):
            ...
            label = get_label(code, label_names)
...
def backtest_stocks(
    ...
    with th.no_grad():
        for code in stk_list:
            ...
            y_pred = model(x_test).squeeze()[:, 0].cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    model_param = {
        ...
        "loss_fn": "mse",
        "weight_decay": 1e-3,
        "loss_weights": [0.6] + [0.4] * (len(label_names) - 1),
        "l2": 1e-1,
        "label_names": label_names,
    }

    model = MMOE(**model_param)
    print(model)
    ## Logger details
    ...
    logger = wandb_logger.WandbLogger(
        project="MMOE-jun6" if args.num_stocks == 100 else "MMOE-s",
    )
    ...
    test_callback = ValidationCallback(10, datamodule, args, stk_list, logger)
    trainer = pl.Trainer(
        devices=[args.gpu],
        callbacks=[
            earlystop_callback,
